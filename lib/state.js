...
export function updateMotion(state, dt, W, H) {
  ...
  const targetR = b * Math.min(15, sizeScale);
    ...
    if (aliveN <= 100) {
      // Smooth exponential curve for last 100: start from current sizeScale, up to 15x
      const t = Math.max(0, Math.min(1, (100 - aliveN) / 99));
      const smoothScale = sizeScale + (15 - sizeScale) * Math.pow(t, 0.85); // smooth from current to 15x
