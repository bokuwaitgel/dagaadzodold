  ...
  for (let i=0;i<people.length;i++) {
    ...
    for (let j=i+1;j<people.length;j++) {
      ...
      if (d < minDist) {
        ...
        // collision damage with randomness only (size doesn't affect damage)
        ...
        // Pure randomness (±50%) - size has no effect on damage dealt or taken
        const rand = state.rand || Math.random;
        const pDmg = baseDmg * (0.5 + 1.0 * rand());
        const qDmg = baseDmg * (0.5 + 1.0 * rand());
        if (p.hitCD === 0) {
          // Equal critical hit chance for everyone (5%) regardless of size
          const critChance = 0.05;
          const isCrit = rand() < critChance;
          const finalDmg = isCrit ? pDmg * 2.5 : pDmg;
        ...
        if (q.hitCD === 0) {
          ...
          const critChance = 0.05;
          const isCrit = rand() < critChance;
          const finalDmg = isCrit ? qDmg * 2.5 : qDmg;
