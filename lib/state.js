...
export function createPerson(rand, id, name, img) {
  return {
    id, name, img,
    hp: 90 + rand() * 20, // randomize initial HP (90-110)
    x: 0, y: 0, r: 26,
    alive: true,
    vx: (rand()-0.5) * 40,
    vy: (rand()-0.5) * 40,
    maxSpeed: 40 + rand() * 80, // more random speed variety (40-120)
    hurtTimer: 0,
    ghostTimer: 0,
    scale: 1,
    hitCD: 0,
...
export function updateMotion(state, dt, W, H) {
  ...
  // dynamic size scaling as followers decrease
    // Scale more conservatively with remaining followers; cap at 1.5x max
  ...
  const sizeScale = Math.min(1.5, 1.0 + 0.15 * Math.sqrt(ratio - 1));
  ...
  for (let i=0;i<people.length;i++) {
    ...
    for (let j=i+1;j<people.length;j++) {
      ...
      if (d < minDist) {
        ...
        // collision damage with randomness and size balancing
        const relvx = p.vx - q.vx, relvy = p.vy - q.vy; const relAlongN = Math.abs(relvx*nx + relvy*ny);
        const impact = Math.min(80, relAlongN);
        const baseDmg = 2 + impact * 0.25;
        // Add randomness (±30%) and slight size penalty for larger avatars
        const pSizeRatio = (p.r / (p.baseR || p.r)) || 1;
        const qSizeRatio = (q.r / (q.baseR || q.r)) || 1;
        const rand = state.rand || Math.random;
        const pDmg = baseDmg * (0.7 + 0.6 * rand()) * Math.min(1.2, 1 + (qSizeRatio - 1) * 0.3);
        const qDmg = baseDmg * (0.7 + 0.6 * rand()) * Math.min(1.2, 1 + (pSizeRatio - 1) * 0.3);
        if (p.hitCD === 0) {
          p.hp -= pDmg; p.hurtTimer = 0.2; p.hitCD = 0.22; p.lastHitById = q.id; q.damageDealt += pDmg;
          state.sim.hits.push({ type:'collide', x:p.x, y:p.y, color:'#f6c05c' });
          state.sim.log.push({ t: state.sim.ticks, type:'collision', name: p.name, by: q.name, dmg: Math.round(pDmg) });
          if (p.hp <= 0 && p.alive) {
        ...
        if (q.hitCD === 0) {
          q.hp -= qDmg; q.hurtTimer = 0.2; q.hitCD = 0.22; q.lastHitById = p.id; p.damageDealt += qDmg;
          state.sim.hits.push({ type:'collide', x:q.x, y:q.y, color:'#f6c05c' });
          state.sim.log.push({ t: state.sim.ticks, type:'collision', name: q.name, by: p.name, dmg: Math.round(qDmg) });
          if (q.hp <= 0 && q.alive) {
