...
export function layoutPeople(rand, people, W, H) {
  ...
  // Compute base radius from available area and count to fit everyone comfortably.
  // Target density factor determines how much free space remains between circles.
  const N = people.length;
  const area = Math.max(1, W * H);
  const density = 0.30; // higher density => larger circles (fills stage better)
  const areaPer = (area * density) / N;
  const targetR = Math.sqrt(areaPer / Math.PI);
  // Clamp to reasonable bounds to keep visuals consistent across sizes
  const base = Math.max(18, Math.min(52, targetR));
  ...
  const margin = Math.max(16, base + 6);
  const placed = [];
  for (let i = 0; i < people.length; i++) {
    const p = people[i];
    let placedOk = false;
...
export function updateMotion(state, dt, W, H) {
  ...
  const sizeScale = Math.max(1, Math.min(2.0, initial / aliveN));
