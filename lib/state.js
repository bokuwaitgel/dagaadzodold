...
export function layoutPeople(rand, people, W, H) {
  ...
  if (N > 4000) globalShrink = 0.55;
  if (N > 6000) globalShrink = 0.52;
  if (N > 8000) globalShrink = 0.50;
  if (N > 12000) globalShrink = 0.48;
  for (let i = 0; i < people.length; i++) {
    const p = people[i];
...
export function updateMotion(state, dt, W, H) {
  ...
  // Much softer growth: approach +35% max instead of doubling
  // const sizeScale = 1 + Math.min(0.35, 0.25 * Math.log10(Math.max(1, ratio)));
  const sizeScale = Math.min(100.0, 1.0 + 0.4 * Math.sqrt(ratio - 1));
  ...
  const b = p.baseR || p.r;
  // Lower minimum for extremely dense cases
  p.r = Math.max(b * 0.6, b * sizeScale);
  }
  // Spatial-hash broadphase collision handling
  ...
  if (N > 0) {
    ...
    let maxR = 6;
    for (let i = 0; i < N; i++) { const p = people[i]; idMap[p.id] = p; if (p.alive && p.r > maxR) maxR = p.r; }
    const cell = Math.max(24, Math.ceil(maxR * 2 + 4));
    const grid = Object.create(null);
    for (let i = 0; i < N; i++) {
      const p = people[i]; if (!p.alive) continue;
      const cx = Math.floor(p.x / cell); const cy = Math.floor(p.y / cell);
      const key = cx + ',' + cy; if (!grid[key]) grid[key] = [];
      grid[key].push(i);
    }
    const rand = state.rand || Math.random;
    for (let i = 0; i < N; i++) {
      const p = people[i]; if (!p.alive) continue;
      const pcx = Math.floor(p.x / cell); const pcy = Math.floor(p.y / cell);
      for (let ox = -1; ox <= 1; ox++) {
        for (let oy = -1; oy <= 1; oy++) {
          const bucket = grid[(pcx + ox) + ',' + (pcy + oy)]; if (!bucket) continue;
          for (let bi = 0; bi < bucket.length; bi++) {
            const j = bucket[bi]; if (j <= i) continue;
            const q = people[j]; if (!q || !q.alive) continue;
            const dx = p.x - q.x; const dy = p.y - q.y; const d2 = dx*dx + dy*dy; if (d2 < 1) continue; const d = Math.sqrt(d2);
            const minDist = p.r + q.r; if (d >= minDist) continue;
            ...
            p.vx += nx * dvp_n; p.vy += ny * dvp_n; q.vx += nx * dvq_n; q.vy += ny * dvq_n;
            const relvx = p.vx - q.vx; const relvy = p.vy - q.vy; const relAlongN = Math.abs(relvx * nx + relvy * ny);
            const impact = Math.min(80, relAlongN);
            const baseDmg = 10 + impact * 0.1;
            const critChance = 0.02;
            if (p.hitCD === 0) {
              const pVar = baseDmg * (0.9 + 0.2 * rand());
              const pCrit = rand() < critChance; const pFinal = pCrit ? pVar * 1.5 : pVar;
              p.hp -= pFinal; p.hurtTimer = 0.2; p.hitCD = 0.22; p.lastHitById = q.id; q.damageDealt += pFinal;
              localHits.push({ type: 'collide', x: p.x, y: p.y, color: pCrit ? '#ff4444' : '#f6c05c' });
              localLog.push({ t: state.sim.ticks, type: 'collision', name: p.name, by: q.name, dmg: Math.round(pFinal), crit: pCrit });
              if (p.hp <= 0 && p.alive) {
                potentialDeaths.push({ personId: p.id, killerId: p.lastHitById });
              }
            }
            if (q.hitCD === 0) {
              const qVar = baseDmg * (0.9 + 0.2 * rand());
              const qCrit = rand() < critChance; const qFinal = qCrit ? qVar * 1.5 : qVar;
              q.hp -= qFinal; q.hurtTimer = 0.2; q.hitCD = 0.22; q.lastHitById = p.id; p.damageDealt += qFinal;
              localHits.push({ type: 'collide', x: q.x, y: q.y, color: qCrit ? '#ff4444' : '#f6c05c' });
              localLog.push({ t: state.sim.ticks, type: 'collision', name: q.name, by: p.name, dmg: Math.round(qFinal), crit: qCrit });
              if (q.hp <= 0 && q.alive) {
                potentialDeaths.push({ personId: q.id, killerId: q.lastHitById });
              }
            }
          }
        }
    ...
    // Push batched hits and log
    for (let h = 0; h < localHits.length; h++) state.sim.hits.push(localHits[h]);
    for (let l = 0; l < localLog.length; l++) state.sim.log.push(localLog[l]);
    // resolve potentialDeaths references to actual objects
    for (let k = 0; k < potentialDeaths.length; k++) {
      const e = potentialDeaths[k]; e.person = idMap[e.personId]; e.killer = idMap[e.killerId] || null;
    }
  }  // Process deaths one at a time to prevent simultaneous deaths
  // Use deterministic ordering: lowest HP first, then lowest ID as tiebreaker
  if (potentialDeaths.length > 0) {
    potentialDeaths.sort((a, b) => {
      const hpDiff = a.person.hp - b.person.hp;
      if (Math.abs(hpDiff) < 0.001) { // Same HP
        return a.person.id - b.person.id; // Lowest ID dies first (deterministic)
      }
      return hpDiff; // Lowest HP dies first
    });
    
    // Only process the first death, save others with 1 HP
    const firstDeath = potentialDeaths[0];
    if (firstDeath.person.alive) {
      firstDeath.person.alive = false;
      if (firstDeath.killer) firstDeath.killer.kills += 1;
      state.sim.deaths.push({ x: firstDeath.person.x, y: firstDeath.person.y });
      state.sim.log.push({ t: state.sim.ticks, type: 'eliminate', name: firstDeath.person.name, by: firstDeath.killer?.name });
    }
    
    // Save all others with 1 HP
    for (let i = 1; i < potentialDeaths.length; i++) {
      if (potentialDeaths[i].person.alive && potentialDeaths[i].person.hp <= 0) {
        potentialDeaths[i].person.hp = 1;
      }
    }
  }
