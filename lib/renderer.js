...
export function createRenderer(canvas) {
  ...
  function draw(state) {
    ...
    // avatars
    const disableFloating = state.people.length > 1000;
    state.people.forEach(p => {
      if (!p.alive) return;
      const r = p.r, scale = p.scale || 1;
      const alpha = 1;
      // gentle floating offsets (visual only) - disabled for large crowds
      let fx = p.x, fy = p.y;
      if (!disableFloating) {
        const tt = time * 0.05; // ~waves
        const amp = Math.max(3, r * 0.12);
        fx = p.x + Math.cos(tt + p.id * 0.83) * amp * 0.6;
        fy = p.y + Math.sin(tt * 1.18 + p.id * 1.11) * amp * 0.5;
      }
      ...
      ctx.strokeStyle = p.searchMatch ? '#24d5ff' : '#1b1b1b';
      ctx.lineWidth = p.searchMatch ? 3 : 2;
