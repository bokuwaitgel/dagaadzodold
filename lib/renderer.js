...
export function createRenderer(canvas) {
  ...
  function pushEffectsFromSim(sim) {
    ...
    while (sim.hits.length) {
      ...
      else { addHitBurst(h.x, h.y, h.color || '#f6c05c'); /* no shake on collisions */ }
    }
    while (sim.deaths.length) { const d = sim.deaths.shift(); addDeathBurst(d.x, d.y); shake = Math.min(28, shake + 8); }
  }
